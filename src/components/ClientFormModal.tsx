  ...
  return (
    <div className="modal-backdrop" onClick={onCancel}>
      <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
        <div className="modal-title">{initial ? t("client.edit_title") : t("client.add_title")}</div>

        <div className="field">
          <label>{t("client.full_name")}</label>
          <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder={t("client.full_name")} />
        </div>

        <div className="field">
          <label>{t("client.phone")}</label>
          <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="09XXXXXXXX" inputMode="tel" />
        </div>

        <div className="field">
          <label>{t("client.status")}</label>
          <div className="segmented">
            <button className={status === "new" ? "active" : ""} onClick={() => setStatus("new")} type="button">
              {t("client.status_new")}
            </button>
            <button className={status === "returning" ? "active" : ""} onClick={() => setStatus("returning")} type="button">
        ...
        <div className="modal-actions">
          ...
          <button
            ...
            disabled={!canSave}
            onClick={() => onSave({ fullName: fullName.trim(), phone: phone.trim(), status, frequency, lastServiceDate, notes })}
          >
            {t("common.save")}
          </button>
        </div>
